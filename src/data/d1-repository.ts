...
export class D1QuestionRepository implements QuestionRepository {
  ...
  private async rows(where = "", bindings: unknown[] = [], limit?: number, offset = 0) {
    const sql = `SELECT q.id,q.slug,q.question_text,COALESCE(c.name,q.category_name) category_name,COALESCE(c.slug,'uncategorised') category_slug,q.claimed_status,q.verified_status,q.verification_state,COALESCE(q.featured,0) featured FROM questions q LEFT JOIN categories c ON c.id=q.category_id WHERE q.publication_state='PUBLISHED' ${where} ORDER BY q.featured DESC,q.published_at DESC,q.id ASC${limit ? ` LIMIT ${limit} OFFSET ${offset}` : ""}`;
    return (await this.db.prepare(sql).bind(...bindings).all<QuestionRow>()).results ?? [];
  }
  async list(filters: QuestionFilters = {}) { const clauses: string[] = []; const values: unknown[] = []; if (filters.status) { clauses.push("q.verified_status=?"); values.push(filters.status); } if (filters.category) { clauses.push("c.slug=?"); values.push(filters.category); } const pageSize = filters.pageSize; const offset = pageSize ? Math.max(0, ((filters.page ?? 1) - 1) * pageSize) : 0; const rows = await this.rows(clauses.length ? `AND ${clauses.join(" AND ")}` : "", values, pageSize, offset); return this.summarize(rows); }
  async count(filters: Pick<QuestionFilters, "status" | "category"> = {}) { const clauses = ["q.publication_state='PUBLISHED'"]; const values: unknown[] = []; if (filters.status) { clauses.push("q.verified_status=?"); values.push(filters.status); } if (filters.category) { clauses.push("c.slug=?"); values.push(filters.category); } const result = await this.db.prepare(`SELECT COUNT(*) count FROM questions q LEFT JOIN categories c ON c.id=q.category_id WHERE ${clauses.join(" AND ")}`).bind(...values).first<{ count: number }>(); return result?.count ?? 0; }
