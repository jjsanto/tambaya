...
export class D1QuestionRepository implements QuestionRepository {
  ...
  private async rows(where = "", bindings: unknown[] = [], limit?: number, offset = 0, sort: QuestionFilters["sort"] = "newest") {
    const order = sort === "recently-verified" ? "q.last_verified_at DESC,q.published_at DESC,q.id ASC" : sort === "most-connected" ? "(SELECT COUNT(*) FROM question_relationships r WHERE r.verified=1 AND (r.source_question_id=q.id OR r.target_question_id=q.id)) DESC,q.published_at DESC,q.id ASC" : "q.published_at DESC,q.created_at DESC,q.id ASC";
  ...
  async list(filters: QuestionFilters = {}) { const clauses: string[] = []; const values: unknown[] = []; if (filters.status) { clauses.push("q.verified_status=?"); values.push(filters.status); } if (filters.category) { clauses.push("c.slug=?"); values.push(filters.category); } if (filters.tag) { clauses.push("EXISTS (SELECT 1 FROM question_tags qtf JOIN tags tf ON tf.id=qtf.tag_id WHERE qtf.question_id=q.id AND tf.slug=?)"); values.push(filters.tag); } const pageSize = filters.pageSize; const offset = pageSize ? Math.max(0, ((filters.page ?? 1) - 1) * pageSize) : 0; const rows = await this.rows(clauses.length ? `AND ${clauses.join(" AND ")}` : "", values, pageSize, offset, filters.sort); return this.summarize(rows); }
  async count(filters: Pick<QuestionFilters, "status" | "category" | "tag"> = {}) { const clauses = ["q.publication_state='PUBLISHED'"]; const values: unknown[] = []; if (filters.status) { clauses.push("q.verified_status=?"); values.push(filters.status); } if (filters.category) { clauses.push("c.slug=?"); values.push(filters.category); } if (filters.tag) { clauses.push("EXISTS (SELECT 1 FROM question_tags qtf JOIN tags tf ON tf.id=qtf.tag_id WHERE qtf.question_id=q.id AND tf.slug=?)"); values.push(filters.tag); } const result = await this.db.prepare(`SELECT COUNT(*) count FROM questions q LEFT JOIN categories c ON c.id=q.category_id WHERE ${clauses.join(" AND ")}`).bind(...values).first<{ count: number }>(); return result?.count ?? 0; }
  ...
  async search(query: string) { if (!query.trim()) return this.list(); const terms = query.match(/[\p{L}\p{N}]+/gu) ?? []; if (!terms.length) return []; const ftsQuery = terms.map(term => `"${term}"`).join(" AND "); const ids = (await this.db.prepare("SELECT question_id FROM question_search WHERE question_search MATCH ? ORDER BY rank LIMIT 50").bind(ftsQuery).all<{question_id:string}>()).results ?? []; if (!ids.length) return []; const placeholders = ids.map(() => "?").join(","); const rows = await this.rows(`AND q.id IN (${placeholders})`, ids.map(item => item.question_id)); return this.summarize(rows); }
