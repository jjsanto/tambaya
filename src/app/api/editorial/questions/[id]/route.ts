...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const [sectionsResult, paragraphsResult, revisionsResult] = await Promise.all([
  ...
  return Response.json({ question: { ...question, sections: (sectionsResult.results ?? []).map(section => ({ key: section.section_key, kicker: section.kicker, title: section.title, paragraphs: paragraphs.filter(item => item.section_id === section.id).map(item => item.body) })), revisions: revisionsResult.results ?? [] } });
}
...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (body.action === "save_story") {
    if (draft.publication_state !== "DRAFT") return Response.json({ error: "Published Stories cannot be edited in place. Create a revision draft first." }, { status: 409 });
