...
export async function POST(request:Request){
  ...
  try{await replaceSubmissionContent(db,id,input,action==="submit"?"SUBMITTED":"DRAFT");}catch{await db.prepare("DELETE FROM questions WHERE id=? AND publisher_id=?").bind(id,user.id).run();return Response.json({error:"Choose a valid category."},{status:400});}
