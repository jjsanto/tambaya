...
export async function POST(request: Request) {
  ...
  const [db, user, raw] = await Promise.all([
    getAuthDatabase(),
  ...
  if (!db || !user)
    return Response.json(
      { error: "Log in to publish a question." },
      { status: 401 },
    );
  ...
  if (!input)
    return Response.json(
      {
        error:
          "Write a complete question ending in a question mark and choose a category.",
      },
      { status: 400 },
    );
  const action = (raw as { action?: string }).action;
  if (action === "submit") {
    const error = validateForSubmission(input);
  ...
  try {
    ...
    await db
      .prepare("DELETE FROM questions WHERE id=? AND publisher_id=?")
      .bind(id, user.id)
      .run();
    return Response.json(
  ...
  return Response.json(
    { id, state: action === "submit" ? "SUBMITTED" : "DRAFT" },
    { status: 201 },
  );
}
