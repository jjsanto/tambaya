...
export type KVNamespaceLike = { put(key:string,value:ArrayBuffer|ReadableStream|string,options?:{metadata?:Record<string,string|number>}):Promise<void>; getWithMetadata<T=Record<string,unknown>>(key:string,options:{type:"stream"}):Promise<{value:ReadableStream|null;metadata:T|null}> };
