...
export type AuthUser = { id: string; username: string; plan:"FREE"|"PREMIUM"; avatar_type:string; avatar_value:string };
...
export async function getCurrentUser(): Promise<AuthUser | null> {
  ...
  return db.prepare("SELECT u.id,u.username,u.plan,u.avatar_type,u.avatar_value FROM auth_sessions s JOIN users u ON u.id=s.user_id WHERE s.id=? AND s.expires_at>CURRENT_TIMESTAMP").bind(digest).first<AuthUser>();
}
...
export async function getRequestUser(request: Request): Promise<AuthUser | null> {
  ...
  const db = await getAuthDatabase();
  if (!token || !db) return null;
  return db.prepare("SELECT u.id,u.username,u.plan,u.avatar_type,u.avatar_value FROM auth_sessions s JOIN users u ON u.id=s.user_id WHERE s.id=? AND s.expires_at>CURRENT_TIMESTAMP").bind(await digestSessionToken(token)).first<AuthUser>();
}
