...
export async function replaceSubmissionContent(db: D1DatabaseLike, id: string, userId:string, input: SubmissionInput, state: SubmissionState) {
  ...
  const statements: D1Statement[] = [
    ...
    db.prepare("UPDATE question_uploads SET question_id=NULL WHERE question_id=? AND owner_id=?").bind(id,userId),
  ...
  input.sections.flatMap(section=>section.blocks).filter((block):block is Extract<StoryBlock,{type:"IMAGE"}>=>block.type==="IMAGE").map(block=>block.src.match(/^\/api\/uploads\/([a-f0-9-]{36}\.(?:jpg|png|webp|gif))$/)?.[1]).filter((key):key is string=>!!key).forEach(key=>statements.push(db.prepare("UPDATE question_uploads SET question_id=? WHERE object_key=? AND owner_id=?").bind(id,key,userId)));
