...
type EditorialDetail = EditorialQuestion & { sections: StoryEditorSection[]; revisions: { id: string; action: string; created_at: string }[] };
...
export function EditorialWorkspace() {
  ...
  async function saveStory(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      await api(`/api/editorial/questions/${editing.id}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "save_story", sections: editorSections }) });
      ...
      <div className="editorial-grid">
        ...
        <section className="editorial-queue"><span className="eyebrow">Review queue</span><h2>Questions in D1</h2>
          {editing && <form className="story-editor" onSubmit={saveStory}><div className="story-editor-head"><div><small>Private Story revision</small><h3>{editing.question_text}</h3></div><button type="button" className="text-link" onClick={() => setEditing(null)}>Close</button></div>
            ...
            <div className="story-editor-controls"><button type="button" className="button ghost small" onClick={() => setEditorSections(current => [...current,{ key:`section-${current.length + 1}`,kicker:"Context",title:"",paragraphs:[""] }])}>Add section</button><button className="button small" disabled={busy}>Save revision</button></div>{editing.revisions.length > 0 && <small>{editing.revisions.length} recent revision event{editing.revisions.length === 1 ? "" : "s"} retained.</small>}
          </form>}
