...
export function EditorialWorkspace({ connected = false }: { connected?: boolean }) {
  ...
  async function rejectQuestion(question:EditorialQuestion){const reviewNotes=window.prompt("Why is this question unsuitable for Tambaya? This decision is final for this submission.");if(!reviewNotes)return;if(!window.confirm(`Reject “${question.question_text}”? The publisher will not be able to edit or resubmit it.`))return;setBusy(true);try{await api(`/api/editorial/questions/${question.id}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({action:"reject",reviewNotes})});setMessage("The question was rejected and the publisher can view the reason.");await load();}catch(error){setMessage(error instanceof Error?error.message:"Unable to reject the question.");}finally{setBusy(false);}}
        ...
        <section className="editorial-queue"><span className="eyebrow">{scope==="review"?"Publisher submissions":"Published records"}</span><h2>{scope==="review"?"Awaiting editorial review":"Question archive"}</h2>
          ...
          {questions.map(question => <article key={question.id} className={`editorial-record ${question.publication_state.toLowerCase()}`}>
            ...
            {question.publication_state === "DRAFT" ? <div className="editorial-actions">
              ...
              <button className="button ghost small" type="button" disabled={busy} onClick={() => void openEditor(question)}>Edit Story</button>{question.submission_state==="SUBMITTED"&&<><button className="button ghost small" type="button" disabled={busy} onClick={()=>void requestChanges(question)}>Request changes</button><button className="button ghost small reject-action" type="button" disabled={busy} onClick={()=>void rejectQuestion(question)}>Reject</button></>}
