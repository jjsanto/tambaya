...
export async function Header() {
  ...
  return <header className="site-header"><div className="shell header-inner">
    ...
    {user ? <><Link className="text-link" href="/circles">Circles</Link><Link className="text-link" href="/account/activity">Activity</Link><Link className="text-link" href="/account/submissions">Submissions</Link><Link className="button small" href="/account">{user.username}</Link></> : <Link className="button small" href="/login">Log in</Link>}
  </div></header>;
}
